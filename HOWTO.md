# HOWTO — install and use Heirloom Development Tools on Darwin
...
```sh
git clone https://github.com/moonman81/heirloom-devtools-darwin
cd heirloom-devtools-darwin
```
...
Gunnar Ritter maintained. Nothing in `heirloom-devtools-darwin` alters
...
writes `.install-manifest-devtools.txt` at the repo root. That
...
Reads `.install-manifest-devtools.txt`, removes every listed file
